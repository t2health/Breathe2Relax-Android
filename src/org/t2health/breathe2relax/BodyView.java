...
public class BodyView extends ScrollView implements Observer {
	...
	public BodyView( Context context, AttributeSet attributeSet, int stuff ){
	...
	private void setBodyImage() {
		try {
			mBodyBitmap = B2R_Utility.getReducedBitmap(getResources(), BODY_PARTS.BODY.getBitmapOrder(), BODY_PARTS.BODY.getBitmapInt());
	...
	private void calculateAspectQuotient() {
		if (mBodyBitmap != null) {
			mAspectQuotient = (((float)mBodyBitmap.getWidth()) / mBodyBitmap.getHeight()) / (((float)getWidth()) / getHeight());
		}
	}
	...
	protected void onDraw(Canvas canvas) {
		if ( mState != null) {
			...
			if (mBodyBitmap != null) {
				final int bitmapWidth = mBodyBitmap.getWidth();
				final int bitmapHeight = mBodyBitmap.getHeight();
				...
				canvas.drawBitmap(mBodyBitmap, mRectSrc, mRectDst, mPaint);
			} 
		}
	}
